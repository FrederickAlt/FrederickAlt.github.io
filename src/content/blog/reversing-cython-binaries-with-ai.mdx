...
The first challenge was getting the correct header files. Since in the binary there are only data bytes and ida pro has no way of knowing the correct structure layouts (except for standard types), i had to get the correct CPython headers. Using `string`, `file`, `readelf`, etc. i could determine that the binary was built for MIPS architecture, used Cpython 3.9, Cython 0.29.* (ancient) and what c-library it was linked against. I guessed the compiler but after some trial and error got a working header file.
